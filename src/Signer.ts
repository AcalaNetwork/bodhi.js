...
export class Wallet
  ...
  constructor(
    ...
    // polkadot
    defineReadOnly(this, 'signerAddress', signerAddress);
  ...
  async claimEvmAccounts() {
    const data =
      'acala evm:' +
      Buffer.from(decodeAddress(this.signerAddress, true)).toString('hex');
    ...
    return new Promise<void>((resolve, reject) => {
      extrinsic.signAndSend(this.signerAddress, (result: SubmittableResult) => {
        handleTxResponse(result, this.provider.api)
          .then(() => {
            resolve();
          })
