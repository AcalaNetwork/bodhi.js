...
CMD yarn install --immutable; yarn e2e:feed-tx
...
WORKDIR /app
ENV ENDPOINT_URL=ws://mandala-node:9944
CMD yarn install --immutable; yarn e2e:feed-tx-2
...
CMD yarn install --immutable; yarn e2e:feed-tx; yarn e2e:feed-tx-2; yarn workspace @acala-network/eth-rpc-adapter run test:CI
