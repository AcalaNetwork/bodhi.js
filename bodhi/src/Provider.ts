...
import { hexToU8a, isNumber, numberToHex, u8aConcat, u8aFixLength } from '@polkadot/util';
...
export class Provider implements AbstractProvider {
  ...
  async estimateResources(transaction: Deferrable<TransactionRequest>): Promise<{
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const result = await (this.api.rpc as any).evm.estimateResources(resolved.from, extrinsic.toHex());
  ...
  async _resolveTransactionReceipt(
    ...
    const result = events.find(
      (x) => x.section.toUpperCase() === 'SYSTEM' && x.method.toUpperCase() === 'EXTRINSICSUCCESS'
    );
  ...
  _resolveBlockHash = async (blockTag?: BlockTag | Promise<BlockTag>): Promise<string> => {
    await this.resolveApi;
  ...
  async _resolveBlockNumber(blockTag?: BlockTag | Promise<BlockTag>): Promise<number | undefined> {
    await this.resolveApi;
  ...
  async _resolveAddress(addressOrName: string | Promise<string>): Promise<string> {
  ...
  async _toAddress(addressOrName: string | Promise<string>): Promise<string> {
    const resolved = await addressOrName;
    const address = encodeAddress(u8aFixLength(u8aConcat('evm:', hexToU8a(resolved)), 256, true));
    return address.toString();
  }

  async _resolveEvmAddress(addressOrName: string | Promise<string>): Promise<string> {
    const resolved = await addressOrName;
  ...
  async _resolveTransaction(transaction: Deferrable<TransactionRequest>): Promise<Deferrable<TransactionRequest>> {
  ...
  async queryContractInfo(
    ...
    if (accountInfo.isNone) {
      return this.api.createType('Option<EvmContractInfo>' as any, null) as Option<EvmContractInfo>;
    }
