...
describe('eth_getTransactionReceipt', () => {
  ...
  it('returns correct result when hash exist for local transactions', async () => {
    ...
    expect(res.data.result).to.deep.equal({
      ...
      effectiveGasPrice: BigNumber.from(txR.effectiveGasPrice).toHexString(),
      status: '0x1',
      type: '0x0'
    });
...
describe('eth_getTransactionByHash', () => {
  ...
  it('finds correct tx when hash exist for local transactions', async () => {
    ...
    expect(res.data.result).to.deep.equal({
      ...
      blockNumber: '0xa',
      transactionIndex: '0x0',
      gasPrice: BigNumber.from(tx1.effectiveGasPrice).toHexString(),
      gas: '0x1e8481',
      input:
    ...
    expect(res.data.result).to.deep.equal({
      ...
      blockNumber: '0x9',
      transactionIndex: '0x0',
      gasPrice: BigNumber.from(tx2.effectiveGasPrice).toHexString(),
      gas: '0x1e8481',
      input:
    ...
    expect(res.data.result).to.deep.equal({
      ...
      blockNumber: '0x6',
      transactionIndex: '0x0',
      gasPrice: BigNumber.from(tx3.effectiveGasPrice).toHexString(),
      gas: '0x1e8481',
      input:
