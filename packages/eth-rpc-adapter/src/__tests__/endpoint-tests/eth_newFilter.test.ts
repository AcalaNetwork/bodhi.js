...
// using either poll method should has same result
const testEthNewFilter = (pollMethod: 'eth_getFilterChanges' | 'eth_getFilterLogs') => {
  describe(`eth_newFilter with ${pollMethod}`, () => {
    let token: Contract;
    ...
    const pollFilter = (filterId: string) => (
      pollMethod === 'eth_getFilterChanges'
    ...
    it('poll immediately', async () => {
      const res1 = (await pollFilter(fid1)).data.result;
      const res2 = (await pollFilter(fid2)).data.result;
      const res3 = (await pollFilter(fid3)).data.result;
    ...
    it('get correct result', async () => {
      ...
      let res1 = (await pollFilter(fid1)).data.result;
      let res2 = (await pollFilter(fid2)).data.result;
      let res3 = (await pollFilter(fid3)).data.result;
      ...
      const res0 = (await pollFilter(fid0)).data.result;
      res1 = (await pollFilter(fid1)).data.result;
      res2 = (await pollFilter(fid2)).data.result;
      res3 = (await pollFilter(fid3)).data.result;
    ...
    it('unsubscribe works', async () => {
      ...
      const res0 = await pollFilter(fid0);
      expect(res0.data.error.message).to.contains('filter not found');
    });

    it.skip('throws correct error message', async () => {
      // tested in eth_newBlockFilter
    });
  });
};

testEthNewFilter('eth_getFilterChanges');
testEthNewFilter('eth_getFilterLogs');
